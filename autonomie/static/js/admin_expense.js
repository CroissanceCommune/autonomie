...
var ExpenseList = {
    ...
    setExpenseJustified: function(){
        var input_tag = $(this);
        var btngroup = input_tag.parent().parent();
        var tr_tag = btngroup.parent().parent();

        var url = btngroup.data('href');
        console.log("Url : %s", url);

        var value = $(this).val();
        var req = ajax_request(url, {'submit': value}, 'POST');
        req.done(function(result){
            var css = "justified-" + capitalize(value);
            tr_tag.removeClass('justified-True');
            tr_tag.removeClass('justified-False');
            tr_tag.addClass('justified-' + value);
        });
    },
